...
export const ImportSettings: React.FC<ImportSettingsProps> = ({ toggles, handleToggle }) => {
    ...
    return (
        <div className="bg-white w-full border-t border-b border-gray-100 relative">
            ...
            <div className="space-y-0">
                ...
                <div className={`overflow-hidden transition-all duration-300 border-b border-gray-100 ${expandedSection === 'global' ? 'bg-indigo-50/5' : 'bg-white hover:bg-gray-50/30'}`}>
                    <button 
                        onClick={() => toggleSection('global')}
                        className="w-full flex items-center justify-between p-4 text-left transition-colors group"
                    >
                        <div className="flex items-center gap-4">
                            <div className={`p-2 rounded-xl transition-all border ${expandedSection === 'global' ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-100' : 'bg-gray-50 text-gray-400 border-gray-100 group-hover:bg-white group-hover:text-indigo-600 group-hover:border-indigo-100'}`}>
                                <Landmark size={16} />
                            </div>
                            <div>
                                <span className={`text-[11px] font-black tracking-wide uppercase ${expandedSection === 'global' ? 'text-indigo-700' : 'text-gray-800'}`}>Global AI Import Rules</span>
                                <p className="text-[10px] text-gray-400 font-medium">Standard behavior across all voucher types</p>
                            </div>
                        </div>
                        <div className={`p-1 rounded-full transition-transform duration-300 ${expandedSection === 'global' ? 'rotate-180 bg-indigo-100 text-indigo-600' : 'bg-gray-50 text-gray-400'}`}>
                            <ChevronDown size={14} />
                        </div>
                    </button>
                    
                    {expandedSection === 'global' && (
                        <div className="px-5 pb-6 bg-white animate-in fade-in slide-in-from-top-1 duration-300">
                            <div className="space-y-2">
                                <div className="flex items-center justify-between gap-4 py-2 border-b border-gray-100/50">
                                    <div>
                                        <p className="text-xs text-gray-800 font-bold">Auto-create missing items</p>
                                    ...
                                    <div onClick={() => handleToggle('autoCreateMissing')} className={`${toggles.autoCreateMissing ? 'bg-indigo-600' : 'bg-gray-300'} w-10 h-5 rounded-full relative cursor-pointer transition-all shrink-0`}>
                                        <div className={`bg-white w-3 h-3 rounded-full absolute top-1 ${toggles.autoCreateMissing ? 'right-1' : 'left-1'} shadow-sm transition-all`}></div>
                                    </div>
                                </div>
                                <div className="flex items-center justify-between gap-4 py-2 border-b border-gray-100/50">
                                    <div>
                                        <p className="text-xs text-gray-800 font-bold">Auto-match ledgers by GSTIN</p>
                                    ...
                                    <div onClick={() => handleToggle('autoMatchLedgerGstin')} className={`${toggles.autoMatchLedgerGstin ? 'bg-indigo-600' : 'bg-gray-300'} w-10 h-5 rounded-full relative cursor-pointer transition-all shrink-0`}>
                                        <div className={`bg-white w-3 h-3 rounded-full absolute top-1 ${toggles.autoMatchLedgerGstin ? 'right-1' : 'left-1'} shadow-sm transition-all`}></div>
                                    </div>
                                </div>
                                <div className="flex items-center justify-between gap-4 py-2 border-b border-gray-100/50">
                                    <div>
                                        <p className="text-xs text-gray-800 font-bold">Smart Narration Cleanup</p>
                                    ...
                                    <div onClick={() => handleToggle('smartNarrationCleanup' as any)} className={`${(toggles as any).smartNarrationCleanup ? 'bg-indigo-600' : 'bg-gray-300'} w-10 h-5 rounded-full relative cursor-pointer transition-all shrink-0`}>
                                        <div className={`bg-white w-3 h-3 rounded-full absolute top-1 ${(toggles as any).smartNarrationCleanup ? 'right-1' : 'left-1'} shadow-sm transition-all`}></div>
                                    </div>
                                </div>
                                <div className="flex items-center justify-between gap-4 py-2">
                                    <div>
                                        <p className="text-xs text-gray-800 font-bold">FileName Date Extraction</p>
                                    ...
                                    <div onClick={() => handleToggle('extractDateFromFileName' as any)} className={`${(toggles as any).extractDateFromFileName ? 'bg-indigo-600' : 'bg-gray-300'} w-10 h-5 rounded-full relative cursor-pointer transition-all shrink-0`}>
                                        <div className={`bg-white w-3 h-3 rounded-full absolute top-1 ${(toggles as any).extractDateFromFileName ? 'right-1' : 'left-1'} shadow-sm transition-all`}></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="border-t border-gray-100">
                    <div className="grid grid-cols-1 divide-y divide-gray-100">
                        {importTypes.map((type) => (
                            <div key={type.id} className={`overflow-hidden transition-all duration-300 ${expandedSection === type.id ? 'bg-indigo-50/5' : 'bg-white hover:bg-gray-50/30'}`}>
                                <button 
                                    onClick={() => toggleSection(type.id)}
                                    className="w-full flex items-center justify-between p-4 text-left transition-colors group"
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={`p-2 rounded-xl transition-all border ${expandedSection === type.id ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-100' : 'bg-gray-50 text-gray-400 border-gray-100 group-hover:bg-white group-hover:text-indigo-600 group-hover:border-indigo-100'}`}>
                                            {type.icon}
                                        </div>
                                        <div>
                                            <span className={`text-[11px] font-black tracking-wide uppercase ${expandedSection === type.id ? 'text-indigo-700' : 'text-gray-800'}`}>{type.title}</span>
                                        </div>
                                    </div>
                                    <div className={`p-1 rounded-full transition-transform duration-300 ${expandedSection === type.id ? 'rotate-180 bg-indigo-100 text-indigo-600' : 'bg-gray-50 text-gray-400'}`}>
                                        <ChevronDown size={14} />
                                    </div>
                                </button>
