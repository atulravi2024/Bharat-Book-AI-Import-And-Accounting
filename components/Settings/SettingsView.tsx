...
import { SettingsIcon, CheckCircleIcon, AccountIcon, NotificationsIcon, SecurityIcon, InfoIcon, AIToolsIcon, AdminIcon, CodeIcon, LayoutIcon } from '../icons/IconComponents';
...
import { BANK_SHORT_CODES, BANK_IGNORE_WORDS } from '../../services/matching/bankCodes';
import { PAYMENT_MODES, PAYMENT_CHANNELS } from '../../services/matching/paymentModes';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ setView, setActiveMasterTab, activeSamples, onToggleSample, setReportBankActiveTab, defaultTab, onTabChange }) => {
    ...
    const [customMappingRules, setCustomMappingRules] = useState<{id: string, keyword: string, targetField: 'partyName' | 'ledger' | 'type', targetValue: string, isRegex?: boolean}[]>([]);
    ...
    const [sandboxResult, setSandboxResult] = useState<{ 
        partyKey: string, personName: string, category: string, reference: string, status: string, color: string,
        mobile: string, bankAccount: string, bankName: string, txnType: string,
        voucherType: string, senderReceiverDetails: string, confidence: string,
        partyKeyConf: number, personNameConf: number, categoryConf: number, referenceConf: number,
        mobileConf: number, bankAccountConf: number, bankNameConf: number, txnTypeConf: number,
        voucherTypeConf: number, senderReceiverDetailsConf: number
    } | null>(null);
    ...
    const runBulkSimulator = (inputs: string[]) => {
        const results = inputs.filter(input => input.trim().length > 0).map(input => {
    ...
    const [activeTab, setActiveTab] = useState<'general' | 'users' | 'alerts' | 'security' | 'privacy' | 'imports' | 'mapping' | 'ai' | 'sample' | 'admin' | 'data' | 'navigation' | 'invoiceprint'>((defaultTab as any) || 'general');
        ...
        <div className="w-full px-2 sm:px-4 py-6 sm:py-8">
            ...
            <div className="flex flex-col space-y-6">
                ...
                <div className="space-y-6">
                    ...
                    {activeTab === 'vouchernumbering' && (
                        <VoucherNumberingSettings />
                    )}

                    {activeTab === 'invoiceprint' && (
                        <InvoicePrintSettings />
                    )}

                    {activeTab === 'users' && <UserSettings />}

                    {activeTab === 'alerts' && <AlertSettings toggles={toggles} handleToggle={handleToggle} />}

                    {activeTab === 'security' && <SecuritySettings />}

                    {activeTab === 'privacy' && <PrivacySettings toggles={toggles} handleToggle={handleToggle} />}
