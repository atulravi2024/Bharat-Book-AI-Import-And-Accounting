      ...
      <table className="w-full text-sm text-left">
        <thead>
          <tr className="bg-gray-50 border-b">
            ...
            <th className="px-4 py-3 font-bold text-gray-600 uppercase text-[10px]">Debit/Credit Ledger</th>
        ...
        <tbody>
          {data.map(v => (
            <tr key={v.id} className="border-b hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-3 text-xs text-gray-500">{String(v.ledger?.value || v.debitLedger?.value || v.creditLedger?.value || '-')}</td>
        ...
        {data.length > 0 && (
          <tfoot>
            <tr className="bg-gray-50 font-bold border-t-2 border-gray-200">
              <td colSpan={4} className="px-4 py-4 text-right uppercase text-[10px] tracking-widest text-gray-500">Total</td>
