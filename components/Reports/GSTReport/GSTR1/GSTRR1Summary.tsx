  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 gap-4">
        <h3 className="text-sm font-bold text-gray-500 uppercase tracking-[0.2em] mb-2 px-1">Section Summaries</h3>
        {['B2B', 'B2C Small', 'B2C Large', 'Export', 'Credit Note', 'Debit Note', 'Exempt'].map((groupType) => {
          ...
          return (
            <div key={groupType} className="bg-white border rounded-xl shadow-sm overflow-hidden flex flex-col transition-all hover:scale-[1.01]">
              <div 
              ...
              >
                <h3 className="font-bold text-gray-800 text-xs uppercase tracking-widest">{groupType} SUMMARY</h3>
                <span className="bg-white/80 backdrop-blur-sm px-3 py-1 rounded-full text-[10px] font-bold border border-gray-100 shadow-sm">
              ...
              <div className="p-5 grid grid-cols-2 md:grid-cols-5 gap-6">
                <div className="flex flex-col">
                  ...
                  <p className="text-base font-bold text-gray-900 font-mono">₹{gTaxable.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                </div>
                <div className="flex flex-col">
                ...
                <div className="flex flex-col">
                  ...
                  <p className="text-sm font-medium text-gray-700 font-mono">₹{gCgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                </div>
                <div className="flex flex-col">
                  <p className="text-[9px] font-black text-gray-400 uppercase tracking-tighter mb-1">SGST</p>
                  <p className="text-sm font-medium text-gray-700 font-mono">₹{gSgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                </div>
                <div className="flex flex-col">
                  <p className="text-[9px] font-black text-gray-400 uppercase tracking-tighter mb-1">IGST</p>
                  <p className="text-sm font-medium text-gray-700 font-mono">₹{gIgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                </div>
              </div>
            </div>
          );
