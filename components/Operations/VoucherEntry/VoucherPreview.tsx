...
import { Printer, Download, Image as ImageIcon, X, ZoomIn, ZoomOut, Maximize, RotateCcw, FileText, GripVertical, ToggleLeft, ToggleRight, Settings, Layout } from 'lucide-react';
...
export const VoucherPreview: React.FC<VoucherPreviewProps> = ({ header, rows, totals, type, onClose, onDownloadPDF, onDownloadImage, autoPrint }) => {
  const isInventory = [
          ...
          <div className="min-h-full flex flex-col justify-center items-center">
            ...
            <style>{`
              ...
              <div 
                id="voucher-to-print" 
                ref={documentRef}
                className={`bg-white flex flex-col min-h-full border border-gray-100 select-text transition-all duration-300 relative ${printConfig.useGrayScale ? 'grayscale contrast-125' : ''} ${isTechnical ? 'border-[3px] border-black' : ''} ${textTransform !== 'default' ? `text-transform-${textTransform}` : ''}`}
                style={{ 
              ...
              <div className={tStyles.tableWrap}>
                <table className="w-full h-full border-collapse">
                    ...
                    <tbody className={tStyles.tableBody}>
                        {rows.filter(r => (isInventory ? r.itemName : r.ledgerName)).map((row, index) => (
