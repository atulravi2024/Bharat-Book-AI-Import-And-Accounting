...
interface AccountingVoucherTabProps {
...
export const AccountingVoucherTab: React.FC<AccountingVoucherTabProps> = ({
    settings,
    collapsedStates,
    handleSettingChange,
    toggleCollapse
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="flex flex-col">
                {voucherTypes.map((type, idx) => {
                    ...
                    const isCollapsed = collapsedStates[type.id] !== false; // collapsed by default
                                    ...
                                    <div className="bg-gray-50 rounded-xl p-5 border border-gray-200/50 dark:bg-gray-950 dark:border-gray-800/80">
                                        {currentSettings.autoGenerate ? (
                                            <>
                                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
                                                    <div className="flex flex-col gap-1.5">
                                                        ...
                                                        <input 
                                                            ...
                                                            className="w-full px-3 py-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg text-xs font-bold text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 shadow-sm transition-all"
                                                            placeholder="e.g. SAL/"
                                                        />
                                                    </div>
                                                    <div className="flex flex-col gap-1.5">
                                                ...
                                                <div className="mt-4 p-3 bg-gradient-to-r from-indigo-50/50 to-indigo-100/10 dark:from-indigo-900/10 dark:to-transparent border border-indigo-100/50 dark:border-indigo-900/30 rounded-xl flex items-center justify-between shadow-xs">
                                                    <div className="flex items-center gap-2 w-full sm:w-auto overflow-hidden">
                                                        <span className="hidden sm:inline-flex items-center justify-center px-2 py-0.5 bg-indigo-50 text-indigo-600 text-[9px] font-black uppercase tracking-widest rounded dark:bg-indigo-950 dark:text-indigo-400 shrink-0">{t("Preview")}</span>
                                                        <div className="font-mono text-xs font-bold text-gray-700 dark:text-gray-300 tracking-wider truncate w-full text-center sm:text-left">
                                                            ...
                                                            <span className="text-indigo-600 dark:text-indigo-400 font-extrabold">{String(currentSettings.startAt || 1).padStart(currentSettings.padding !== undefined ? currentSettings.padding : 3, '0')}</span>
