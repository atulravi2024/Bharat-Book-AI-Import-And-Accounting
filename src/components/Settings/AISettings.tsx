        ...
        <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-premium-slate-100 relative dark:bg-gray-800 dark:border-gray-700">
            ...
            <div className="space-y-8">
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 dark:bg-gray-900 dark:border-gray-800">
                    <h4 className="font-bold text-gray-900 text-sm mb-4 uppercase tracking-widest dark:text-white">Active System</h4>
                    <div className="grid grid-cols-2 gap-4">
                        <button 
                            onClick={() => setAiSettings({...aiSettings, provider: 'internal'})}
                            className={`p-4 rounded-2xl border-2 transition-all text-left ${aiSettings.provider === 'internal' ? 'border-blue-600 bg-blue-50' : 'border-gray-200 bg-white'} dark:border-gray-700 dark:bg-gray-800`}
                        >
                            <p className="font-bold text-sm text-gray-900 dark:text-white">Google Gemini (Internal)</p>
                            <p className="text-[10px] text-gray-500 font-medium dark:text-gray-400">Standard system model. Managed by platform.</p>
                        </button>
                        <button 
                            onClick={() => setAiSettings({...aiSettings, provider: 'external'})}
                            className={`p-4 rounded-2xl border-2 transition-all text-left ${aiSettings.provider === 'external' ? 'border-blue-600 bg-blue-50' : 'border-gray-200 bg-white'} dark:border-gray-700 dark:bg-gray-800`}
                        >
                            <p className="font-bold text-sm text-gray-900 dark:text-white">Custom Provider (External)</p>
                            <p className="text-[10px] text-gray-500 font-medium dark:text-gray-400">Use your own API keys for more control.</p>
                        </button>
                    </div>
                </div>
                ...
                {aiSettings.provider === 'internal' ? (
                    <div className="space-y-6 animate-in slide-in-from-top-4 duration-300">
                        <div>
                            ...
                            <select 
                                className="w-full p-4 bg-premium-slate-50 border-none rounded-2xl font-bold text-gray-700 focus:ring-2 focus:ring-blue-100 outline-none appearance-none dark:bg-gray-800 dark:text-gray-200"
                    ...
                    <div className="space-y-6 animate-in slide-in-from-top-4 duration-300">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input 
                                    type="password" 
                                    className={`w-full p-4 bg-premium-slate-50 border-none rounded-2xl font-bold text-gray-700 focus:ring-2 focus:ring-blue-100 outline-none placeholder:text-gray-300 ${aiSettings.externalProvider === '9router' ? 'opacity-50 cursor-not-allowed' : ''} dark:bg-gray-800 dark:text-gray-200`} 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div className="relative">
                                    <input 
                                        type="text" 
                                        className={`w-full p-4 bg-premium-slate-50 border-none rounded-2xl font-bold text-gray-700 focus:ring-2 focus:ring-blue-100 outline-none ${aiSettings.externalProvider === '9router' ? 'opacity-80' : ''} dark:bg-gray-800 dark:text-gray-200`} 
                                        ...
                                        <div className="mt-2 flex flex-wrap gap-2">
                                            {EXTERNAL_PROVIDERS.find(p => p.id === aiSettings.externalProvider)?.models.map(m => (
                                                <button 
                                                    ...
                                                    className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all ${aiSettings.model === m ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-400 border-gray-200 hover:border-blue-400'} dark:bg-gray-800 dark:border-gray-700`}
                                                >
                                                    {m}
                                                </button>
                                            ))}
                ...
                <div className="pt-4 border-t border-premium-slate-100 space-y-4 dark:border-gray-700">
                    {testStatus.message && (
                    ...
                    <div className="flex justify-between items-center">
                        <p className="text-[10px] text-gray-500 font-bold max-w-xs uppercase leading-relaxed dark:text-gray-400">
                        ...
                        <div className="flex gap-4">
                            <button 
                                ...
                                className={`px-6 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center ${testStatus.loading ? 'bg-gray-100 text-gray-400' : 'bg-premium-slate-100 text-premium-slate-700 hover:bg-premium-slate-200 shadow-sm'} dark:bg-gray-800`}
                            >
