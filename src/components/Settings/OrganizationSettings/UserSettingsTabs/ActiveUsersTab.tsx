...
export const ActiveUsersTab: React.FC<ActiveUsersTabProps> = ({ searchTerm = "" }) => {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    ...
    window.addEventListener('bharat_book_users_imported_trigger', loadUsersFromLocalStorage);
    return () => {
      window.removeEventListener('bharat_book_users_imported_trigger', loadUsersFromLocalStorage);
    };
  }, []);
  ...
  const activeUsers = users.filter(u => {
    const matchesActive = u.status === 'Active';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {activeUsers.length === 0 ? (
          <div className="col-span-1 md:col-span-2 lg:col-span-3 text-center py-10 bg-slate-50 dark:bg-gray-900 rounded-2xl">
            <p className="text-xs text-gray-400 font-bold uppercase tracking-wider">{t("No active users found")}</p>
          </div>
        ) : (
            ...
            <div key={user.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
              ...
              <div className="space-y-3 mb-4">
                <div className="flex flex-col gap-1">
                  <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{t("Email Address")}</span>
                ...
                <div className="flex flex-col gap-1">
                  <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{t("Contact Number")}</span>
                  <div className="flex items-center text-xs font-semibold text-gray-700 dark:text-gray-300">
                    <Phone className="w-3.5 h-3.5 mr-2 text-emerald-500" /> {user.phone}
                  </div>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{t("Department")}</span>
                  <div className="flex items-center text-xs font-semibold text-gray-700 dark:text-gray-300">
                    <Briefcase className="w-3.5 h-3.5 mr-2 text-indigo-500" /> {user.department} Team
                  </div>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{t("Session Status")}</span>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center text-xs font-semibold text-gray-700 dark:text-gray-300">
                      <Activity className="w-3.5 h-3.5 mr-2 text-green-500" /> 
                      <span className="bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400 px-2 py-0.5 rounded text-[10px] uppercase font-black tracking-widest mr-2">{t("Online")}</span>
                      {user.lastActive}
                    </div>
                    {user.id !== 'usr-1' && (
                      <button 
              ...
              <div className="border-t border-gray-100 dark:border-gray-700 pt-3">
                 <h5 className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">{t("Module Access Permissions")}</h5>
                 <div className="grid grid-cols-2 gap-2 text-[10px] font-bold text-gray-600 dark:text-gray-300">
                    <div>
                      <span className="text-gray-400 block mb-1">{t("Vouchers")}</span>
                    ...
                    <div>
                      <span className="text-gray-400 block mb-1">{t("Masters")}</span>
                      <div className="flex gap-1">
