...
export const GroupRulesTab: React.FC<GroupRulesTabProps> = ({ searchTerm = "" }) => {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    loadDataAndStatistics();
    window.addEventListener('bharat_book_users_imported_trigger', loadDataAndStatistics);

    // 2. Load current user context
    const loggedInId = localStorage.getItem('bharat_book_current_logged_in_user_id');
    const savedUsers = localStorage.getItem('bharat_book_managed_users');
    let loadedUsers: ManagedUser[] = savedUsers ? JSON.parse(savedUsers) : INITIAL_USERS;
    if (loggedInId) {
    ...
    return () => {
      window.removeEventListener('bharat_book_users_imported_trigger', loadDataAndStatistics);
    };
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {activePane === 'role' && (Object.entries(rolePolicies) as [string, GroupPolicy][])
          .filter(([roleName]) => !searchTerm ? true : roleName.toLowerCase().includes(searchTerm.toLowerCase()))
          .map(([roleName, policy]) => {
        ...
        {activePane === 'department' && (Object.entries(deptPolicies) as [string, GroupPolicy][])
          .filter(([deptName]) => !searchTerm ? true : deptName.toLowerCase().includes(searchTerm.toLowerCase()))
          .map(([deptName, policy]) => {
