...
export const OperationalSection: React.FC<Props> = ({ firmData, setFirmData, activeAccordion, toggleAccordion, bankOptions, ledgerMasters, searchTerm }) => {
  const { t } = useLanguage();
  const { isFieldVisible, isSectionVisible } = useSearchFilter(searchTerm);

  if (!isSectionVisible("operational")) return null;
  const isExpanded = activeAccordion === "operational" || (Boolean(searchTerm) && isSectionVisible("operational"));
  return (
    <>
...
{isFieldVisible("Working Days") && (
                        ...
                        {firmData.workingDays === "Custom" && isFieldVisible("Custom Working Days") && (
