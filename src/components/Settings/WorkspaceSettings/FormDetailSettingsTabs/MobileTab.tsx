...
export const MobileTab: React.FC<MobileTabProps> = ({ 
  ...
  showMobileShowBorders = true,
  showMobileSticky = true
}) => {
  const { t } = useLanguage();
  const updateSetting = (key: string, val: any) => handleSettingChange(key, val);

  const hasMobileErgonomics = showMobileModal;
  const hasDisplayToggles = showMobileShowBorders || showMobileSticky;

  if (!hasMobileErgonomics && !hasDisplayToggles) return null;
