...
interface DesktopTabProps {
  ...
  showDesktopSplit?: boolean;
  showDesktopInputSize?: boolean;
}

export const DesktopTab: React.FC<DesktopTabProps> = ({ 
  settings, 
  handleSettingChange,
  showDesktopLayout = true,
  showDesktopSplit = true,
  showDesktopInputSize = true
}) => {
  const { t } = useLanguage();
  const updateSetting = (key: string, val: any) => handleSettingChange(key, val);
  
  const hasLayoutOptions = showDesktopLayout || showDesktopSplit;
  const hasSizing = showDesktopInputSize;

  if (!hasLayoutOptions && !hasSizing) return null;
