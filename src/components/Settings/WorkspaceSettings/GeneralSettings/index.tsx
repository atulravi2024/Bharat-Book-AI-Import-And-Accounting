...
import { InfoIcon, CheckCircleIcon, UploadIcon, UndoIcon, ClearAllIcon, LayoutIcon, MapIcon, SettingsIcon, SaveIcon, SearchIcon, DownloadIcon } from "../../../icons/IconComponents";
...
export const GeneralSettings: React.FC<GeneralSettingsProps> = ({
  ...
  const handleExportBackup = () => {
    const data = {
      theme, language, dateFormat, timezone, autoLock, density, animations,
      soundEffects, keyboardShortcuts, weekStartsOn, paginationSize, showSystemInfo, displayId, appMode
    };
    if (targetFormat === "json") {
      const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(data, null, 2));
      const downloadAnchor = document.createElement('a');
      downloadAnchor.setAttribute("href", dataStr);
      downloadAnchor.setAttribute("download", "general_settings_backup.json");
      ...
      downloadAnchor.setAttribute("download", "general_settings_backup.csv");
      document.body.appendChild(downloadAnchor);
      downloadAnchor.click();
      downloadAnchor.remove();
    }
  ...
  const handleCombinedImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (parsed.displayId !== undefined) setDisplayId(parsed.displayId);
        if (parsed.appMode !== undefined) setAppMode(parsed.appMode);
        ...
        if (parsed.keyboardShortcuts !== undefined) setKeyboardShortcuts(parsed.keyboardShortcuts);
        if (parsed.weekStartsOn !== undefined) setWeekStartsOn(parsed.weekStartsOn);
        if (parsed.paginationSize !== undefined) setPaginationSize(parsed.paginationSize);
        if (parsed.showSystemInfo !== undefined) setShowSystemInfo(parsed.showSystemInfo);
      } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-white dark:bg-gray-900 p-3.5 rounded-xl border border-gray-200/60 dark:border-gray-800 shadow-sm animate-in fade-in overflow-hidden">
        <div className="flex items-center gap-3 shrink-0 min-w-0 md:max-w-md">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] xs:text-[11px] text-gray-500 dark:text-gray-400 font-medium mt-0.5 truncate whitespace-nowrap" title={t("Manage structural preferences and behaviors")}>
              {t("Manage structural preferences and behaviors")}
            </p>
          </div>
        </div>
