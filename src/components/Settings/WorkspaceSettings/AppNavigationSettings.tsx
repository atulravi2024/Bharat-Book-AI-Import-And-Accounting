...
export const AppNavigationSettings: React.FC = () => {
  ...
  useEffect(() => {
    const checkOverride = () => {
      const override = localStorage.getItem('bharat_book_navigation_subtab_override');
      if (override) {
        if (override === "defaults" || override === "priority") setActiveTab("priority");
        else if (override === "routing" || override === "custom") setActiveTab("routing");
        localStorage.removeItem('bharat_book_navigation_subtab_override');
      }
    };
    checkOverride();
    window.addEventListener('bharat_book_navigation_subtab_trigger', checkOverride);
    return () => window.removeEventListener('bharat_book_navigation_subtab_trigger', checkOverride);
  }, []);
    ...
    reader.onload = (e) => {
      ...
      try {
        if (exportFormat === "JSON") {
          ...
          if (parsed.page) setDefaultPage(parsed.page);
          if (parsed.subPage) setDefaultSubPage(parsed.subPage);
          if (parsed.subSubPage !== undefined) setDefaultSubSubPage(parsed.subSubPage || "");
          if (parsed.routing) setRoutingDefaults(parsed.routing);
          ...
          if (lines.length > 1) {
            const tempRouting: Record<string, string> = { ...routingDefaults };
            lines.forEach((line) => {
              ...
              } else if (key === "defaultSubPage") {
                setDefaultSubPage(value);
              } else if (key === "defaultSubSubPage") {
                setDefaultSubSubPage(value);
              } else {
                tempRouting[key] = value;
              }
            });
            setRoutingDefaults(tempRouting);
            setIsSaved(true);
            setTimeout(() => setIsSaved(false), 2000);
          }
        }
  ...
  const handleExport = () => {
    ...
    let fileName = `navigation_defaults.${exportFormat.toLowerCase()}`;
    ...
    if (exportFormat === "JSON") {
      ...
      const rows = [
        ...
        ["defaultSubSubPage", defaultSubSubPage],
        ...Object.entries(routingDefaults).map(([k, v]) => [k, v]),
      ];
  ...
  const showLanding = isFieldVisible("Main Landing Category", ["Landing", "default page", "Company/Startup Category"]);
  const showSubSection = isFieldVisible("Target Sub-Section", ["Sub-Section", "subpage", "default subpage"]);
  const showFocusView = isFieldVisible("Focus View Subtab", ["Focus View", "sub-subpage", "default sub-subpage"]);

  const priorityMatchCount = searchQuery.trim() ? ((showLanding ? 1 : 0) + (showSubSection ? 1 : 0) + (showFocusView ? 1 : 0)) : 0;

  const routingMatchCount = searchQuery.trim() ? pages.filter(p => {
    const subPageLabels = subPages[p.id]?.map(sp => sp.label) || [];
    return isFieldVisible(p.label, [p.id, ...subPageLabels]);
  }).length : 0;
  ...
  useEffect(() => {
    ...
    if (activeTab === "priority" && priorityMatchCount === 0 && routingMatchCount > 0) {
      setActiveTab("routing");
    } else if (activeTab === "routing" && routingMatchCount === 0 && priorityMatchCount > 0) {
    ...
    <div className="max-w-6xl mx-auto space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center bg-white dark:bg-gray-900 p-3.5 rounded-xl border border-gray-200/60 dark:border-gray-800 shadow-sm animate-in fade-in overflow-hidden justify-between">
        <div className="flex items-center gap-3 shrink-0 sm:max-w-[30%]">
          ...
          <div className="min-w-0">
            <h2 className="text-[15px] font-bold text-gray-900 dark:text-white leading-tight truncate">{t("App Defaults")}</h2>
            <p className="text-[10px] xs:text-[11px] text-gray-500 dark:text-gray-400 font-medium truncate whitespace-nowrap">{t("Configure default startups and sidebar routing rules.")}</p>
          </div>
        </div>
        ...
        <div className="min-w-0 flex-1 flex justify-end">
          <div className="flex items-center bg-gray-100/80 dark:bg-gray-800/80 p-1 rounded-xl gap-1 shadow-sm overflow-x-auto custom-scrollbar w-auto  border border-gray-200/40 dark:border-gray-700/40 shrink-0 justify-end">
             {tabs.map((tab) => {
                const Icon = tab.icon;
                const matchCount = tab.id === 'priority' ? priorityMatchCount : routingMatchCount;
                return (
                  <button
                    key={tab.id}
                  ...
                  >
                    ...
                    {isSearching && (
                      <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-black ${
                        activeTab === tab.id 
                          ? 'bg-blue-105 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300' 
                          : 'bg-gray-200 text-gray-650 dark:bg-gray-700 dark:text-gray-400'
                      }`}>
                        {matchCount}
                      </span>
                    )}
      ...
      <div className="flex flex-row justify-between items-center gap-2 bg-white dark:bg-gray-900 p-1.5 sm:p-2 rounded-xl border border-gray-200/60 dark:border-gray-800 shadow-sm animate-in fade-in overflow-hidden">
        <div className="flex-1 min-w-0 relative">
          ...
          <input 
            type="text" 
            placeholder={t("Search app defaults and routing rules...")} 
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            onFocus={() => setIsSearchFocused(true)}
            onBlur={() => setIsSearchFocused(false)}
            className="w-full pl-8 pr-7 py-1.5 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg text-[11px] font-bold text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-blue-105 dark:focus:ring-blue-900/30 outline-none transition-all placeholder:text-gray-400 dark:placeholder:text-gray-500"
          />
          {searchQuery && (
            <button
        ...
        <div className={`flex-row flex-nowrap items-center justify-end gap-0.5 sm:gap-1 bg-transparent sm:bg-gray-50 dark:sm:bg-gray-900/50 sm:p-1 rounded-xl sm:border sm:border-gray-200 dark:sm:border-gray-700 shrink-0 overflow-x-auto custom-scrollbar ${(isSearchFocused || searchQuery) ? "hidden sm:flex" : "flex"}`}>
          ...
          >
            <DownloadIcon className="!text-[16px] flex items-center justify-center shrink-0" />
          ...
          <button
            onClick={handleReset}
            className="px-2 sm:px-3 py-1.5 text-[11px] font-bold text-gray-650 dark:text-gray-300 hover:text-red-600 hover:bg-white dark:hover:bg-gray-800 rounded-lg transition-colors border border-transparent shadow-sm active:scale-95 flex items-center justify-center gap-1.5 shrink-0"
          ...
          <div className="hidden lg:block w-px h-4 bg-gray-300 dark:bg-gray-700 mx-1"></div>

          {/* Save Button */}
          <button
            onClick={handleSave}
            className={`flex items-center justify-center gap-1.5 px-3.5 py-1.5 rounded-lg text-[11px] font-bold transition-all shadow-sm active:scale-95 shrink-0 ${isSaved ? "bg-emerald-50 text-emerald-600" : "bg-blue-600 text-white hover:bg-blue-700"}`}
          >
      ...
      {activeTab === "priority" && (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl p-6 border border-gray-200/60 dark:border-gray-800 shadow-sm relative overflow-hidden animate-in fade-in duration-300">
          ...
          {(!isSearching || priorityMatchCount > 0) ? (
            ...
            /* ZERO MATCHES WARNING FOR PRIORITY */
            <div className="p-8 flex flex-col items-center justify-center text-center animate-in fade-in duration-300 relative z-10">
              <div className="w-12 h-12 rounded-full bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-600 dark:text-amber-400 border border-amber-100/50 dark:border-amber-500/20 mb-4">
                <SettingsIcon className="!text-[24px]" />
              </div>
              <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-2">
                {t("No matches in Startup Priority")}
              </h3>
              {routingMatchCount > 0 ? (
                <>
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/10 dark:hover:bg-blue-900/20 text-blue-600 dark:text-blue-400 font-bold text-xs rounded-xl transition-all border border-blue-100/30 active:scale-95 shadow-sm"
                  >
                    <AIToolsIcon className="!text-[16px]" />
                    <span>{t("Routing Rules")}</span>
                    <span className="bg-blue-100 dark:bg-blue-900/30 text-[10px] px-2 py-0.5 rounded-full font-black">
                      {routingMatchCount}
                    </span>
                  </button>
                </>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 max-w-md leading-relaxed">
                  {t("Try clearing your query or typing different search terms.")}
                </p>
              )}
            </div>
          )}
      ...
      {activeTab === "routing" && (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl p-6 border border-gray-200/60 dark:border-gray-800 shadow-sm relative overflow-hidden animate-in fade-in duration-300">
          ...
          {(!isSearching || routingMatchCount > 0) ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 relative z-10">
              {pages.map((page) => {
                const subPageLabels = subPages[page.id]?.map(sp => sp.label) || [];
                const isPageVisible = isFieldVisible(page.label, [page.id, ...subPageLabels]);
            ...
            /* ZERO MATCHES WARNING FOR ROUTING */
            <div className="p-8 flex flex-col items-center justify-center text-center animate-in fade-in duration-300 relative z-10">
              <div className="w-12 h-12 rounded-full bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-600 dark:text-amber-400 border border-amber-100/50 dark:border-amber-500/20 mb-4">
                <AIToolsIcon className="!text-[24px]" />
              </div>
              <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-2">
                {t("No matches in Routing Rules")}
              </h3>
