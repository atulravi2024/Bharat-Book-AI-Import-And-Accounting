...
export const GradesTab: React.FC<GradesTabProps> = ({ data, onSave }) => {
        ...
        <div className="flex flex-col h-full animate-in fade-in duration-300">
            <div className="p-4 bg-gray-50/30 border-b border-gray-100 flex justify-between items-center dark:bg-gray-800/30 dark:border-gray-800">
                <div className="relative max-w-md w-full mr-4">
                    ...
                    <input type="text" placeholder={t("Search Grades...")} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="form-input pl-10 pr-4 text-sm" />
                </div>
                <div className="flex items-center">
                    <ImportExportButtons data={data} onSave={onSave} entityName="GradesTab" />
                    ...
                    <AddIcon className="lg:mr-2" /> <span className="hidden lg:inline-block">{t("Add Grade")}</span></button>
                </div>
            </div>
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200 dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Quality Score")}</th>
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Standard")}</th>
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Tolerance")}</th>
                    ...
                    <div className={`bg-white w-full h-full overflow-hidden flex flex-col dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-2xl animate-in zoom-in-95 ${formSettings.currentModalMode === 'fullscreen' ? 'rounded-none max-w-full max-h-full' : 'rounded-2xl max-w-5xl max-h-[90vh]'}`}>
                        <div className="flex justify-between items-center px-4 py-2 border-b border-gray-100 bg-gray-50/50 dark:border-gray-800">
                            <h2 className="font-bold text-base text-gray-900 flex items-center dark:text-white">
                                {editingId ? t('Edit') : t('Add')} {t('Grade')}
                            </h2>
...
<input type="text" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} className="form-input" placeholder={t("Enter name...")} />
                                ...
                                <div className="form-field-wrapper col-span-1">
                                    <label className="form-label dark:text-gray-400">{t("Standard")}</label>
                                    <input type="text" value={formData.standard || ''} onChange={e => setFormData({...formData, standard: e.target.value})} className="form-input bg-transparent" placeholder={t("e.g. ISO, DIN, ASTM...")} />
                                </div>
                                <div className="form-field-wrapper col-span-1">
                                    <label className="form-label dark:text-gray-400">{t("Tolerance")}</label>
                                    <input type="text" value={formData.tolerance || ''} onChange={e => setFormData({...formData, tolerance: e.target.value})} className="form-input bg-transparent" placeholder={t("e.g. ±0.1mm, Class 2...")} />
                                </div>
        <div className="form-field-wrapper">
<label className="form-label dark:text-gray-400">{t("Quality Score (1-10)")}</label><input type="number" min="1" max="10" value={formData.qualityScore || ''} onChange={e => setFormData({...formData, qualityScore: parseFloat(e.target.value)})} className="form-input bg-transparent" placeholder={t("Score...")} /></div>
            ...
            {deleteConfirmation?.isOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95 dark:bg-gray-800">
                        ...
                        <h2 className="font-bold text-xl mb-2 text-gray-900 dark:text-white">{t("Delete Grade?")}</h2>
