...
import { AssertionCategoryMaster } from '../../../../app/types';


interface AssertionCodesTabProps {
    data: any[];
    onSave: (items: any[]) => void;
    assertionCategoryMasters: AssertionCategoryMaster[];
}

export const AssertionCodesTab: React.FC<AssertionCodesTabProps> = ({ data, onSave, assertionCategoryMasters }) => {
  ...
  const formSettings = useFormSettings();
    ...
    const getCategoryName = (categoryId: string) => {
        const category = assertionCategoryMasters?.find(c => c.id === categoryId);
        ...
        <div className="flex flex-col h-full animate-in fade-in duration-300">
            <div className="p-4 bg-gray-50/30 border-b border-gray-100 flex justify-between items-center dark:bg-gray-800/30 dark:border-gray-800">
                <div className="relative max-w-md w-full mr-4">
                    ...
                    <input type="text" placeholder={t("Search Assertion Codes...")} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="form-input pl-10 pr-4 text-sm" />
                </div>
                <div className="flex items-center">
                    <ImportExportButtons data={data} onSave={onSave} entityName="AssertionCodesTab" />
                    ...
                    <AddIcon className="lg:mr-2" /> <span className="hidden lg:inline-block">{t("Add Code")}</span></button>
                </div>
            </div>
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-gray-800">
                            {filteredData.map((m: any) => (
                                <tr key={m.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                                    ...
                                    <td className="p-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-200">{getCategoryName(m.categoryId)}</td>
                    ...
                    <div className={`bg-white w-full h-full overflow-hidden flex flex-col dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-2xl animate-in zoom-in-95 ${formSettings.currentModalMode === 'fullscreen' ? 'rounded-none max-w-full max-h-full' : 'rounded-2xl max-w-5xl max-h-[90vh]'}`}>
                        <div className="flex justify-between items-center px-4 py-2 border-b border-gray-100 bg-gray-50/50 dark:border-gray-800">
                            <h2 className="font-bold text-base text-gray-900 flex items-center dark:text-white">
                                {editingId ? t('Edit') : t('Add')} {t('Assertion Code')}
                            </h2>
                        ...
                        <div className="overflow-y-auto flex-1 p-6 space-y-4">
                            <div className="form-grid gap-4">
                                <div className="form-field-wrapper col-span-1">
                                    ...
                                    <input type="text" value={formData.code || ''} onChange={e => setFormData({...formData, code: e.target.value})} className="form-input" placeholder={t("Enter code...")} autoFocus />
                                </div>
                                <div className="form-field-wrapper col-span-1">
                                ...
                                <div className="form-field-wrapper col-span-1 md:col-span-2">
                                    ...
                                    <select value={formData.categoryId || ''} onChange={e => setFormData({...formData, categoryId: e.target.value})} className="form-input bg-white dark:bg-gray-800">
                                        <option value="">{t("Select Category...")}</option>
                                        {assertionCategoryMasters?.map(c => (
            ...
            {deleteConfirmation?.isOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95 dark:bg-gray-800">
                        ...
                        <h2 className="font-bold text-xl mb-2 text-gray-900 dark:text-white">{t("Delete Assertion Code?")}</h2>
