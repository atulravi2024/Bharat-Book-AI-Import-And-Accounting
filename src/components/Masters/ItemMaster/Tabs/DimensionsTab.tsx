        ...
        <div className="flex flex-col h-full animate-in fade-in duration-300">
            <div className="p-4 bg-gray-50/30 border-b border-gray-100 flex justify-between items-center dark:bg-gray-800/30 dark:border-gray-800">
                <div className="relative max-w-md w-full mr-4">
                    ...
                    <input type="text" placeholder={t("Search Dimensions...")} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="form-input pl-10 pr-4 text-sm" />
                </div>
                <div className="flex items-center">
                    ...
                    <AddIcon className="lg:mr-2" /> <span className="hidden lg:inline-block">{t("Add Dimension")}</span></button>
                </div>
            </div>
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200 dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Length")}</th>
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Width")}</th>
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Height")}</th>
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Unit")}</th>
                    ...
                    <div className={`bg-white w-full h-full overflow-hidden flex flex-col dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-2xl animate-in zoom-in-95 ${formSettings.currentModalMode === 'fullscreen' ? 'rounded-none max-w-full max-h-full' : 'rounded-2xl max-w-5xl max-h-[90vh]'}`}>
                        <div className="flex justify-between items-center px-4 py-2 border-b border-gray-100 bg-gray-50/50 dark:border-gray-800">
                            <h2 className="font-bold text-base text-gray-900 flex items-center dark:text-white">
                                {editingId ? t('Edit') : t('Add')} {t('Dimension')}
                            </h2>
...
<label className="form-label dark:text-gray-400">{t("Length")}</label><input type="number" value={formData.l || ''} onChange={e => setFormData({...formData, l: parseFloat(e.target.value)})} className="form-input bg-transparent" placeholder={t("Length...")} /></div>
        <div className="form-field-wrapper">
<label className="form-label dark:text-gray-400">{t("Width")}</label><input type="number" value={formData.w || ''} onChange={e => setFormData({...formData, w: parseFloat(e.target.value)})} className="form-input bg-transparent" placeholder={t("Width...")} /></div>
        
        <div className="form-field-wrapper">
<label className="form-label dark:text-gray-400">{t("Height")}</label><input type="number" value={formData.h || ''} onChange={e => setFormData({...formData, h: parseFloat(e.target.value)})} className="form-input bg-transparent" placeholder={t("Height...")} /></div>
        
        <div className="form-field-wrapper">
<label className="form-label dark:text-gray-400">{t("Unit")}</label><input type="text" value={formData.unit || ''} onChange={e => setFormData({...formData, unit: e.target.value})} className="form-input bg-transparent" placeholder={t("cm, in, m...")} /></div>

        <div className="form-field-wrapper col-span-1 md:col-span-2">
            <label className="form-label dark:text-gray-400">{t("Volume (Calculated automatically if left blank)")}</label>
            <input type="number" value={formData.volume || (formData.l * formData.w * formData.h) || ''} onChange={e => setFormData({...formData, volume: parseFloat(e.target.value)})} className="form-input font-mono" placeholder={t("Pre-calculated volume...")} />
        </div>
            ...
            {deleteConfirmation?.isOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95 dark:bg-gray-800">
                        ...
                        <h2 className="font-bold text-xl mb-2 text-gray-900 dark:text-white">{t("Delete Dimension?")}</h2>
