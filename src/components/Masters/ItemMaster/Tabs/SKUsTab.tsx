  ...
  const formSettings = useFormSettings();
    ...
    return (
        <div className="flex flex-col h-full animate-in fade-in duration-300">
            <div className="p-4 bg-gray-50/30 border-b border-gray-100 flex justify-between items-center dark:bg-gray-800/30 dark:border-gray-800">
                <div className="relative max-w-md w-full mr-4">
                    ...
                    <input type="text" placeholder={t("Search SKUs...")} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="form-input pl-10 pr-4 text-sm" />
                </div>
                <div className="flex items-center">
                    <ImportExportButtons data={data} onSave={onSave} entityName="SKUsTab" />
                    ...
                    <AddIcon className="lg:mr-2" /> <span className="hidden lg:inline-block">{t("Add SKU")}</span></button>
                </div>
            </div>
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200 dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("SKU Code")}</th>
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Item Code")}</th>
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Barcode")}</th>
        ...
        <td className="p-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-200 font-mono">
            {m.itemCode && itemMasters?.find((i: any) => i.id === m.itemCode) ? itemMasters.find((i: any) => i.id === m.itemCode)?.name : m.itemCode}
        </td>
...
<input type="text" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} className="form-input" placeholder={t("Enter name...")} />
                                ...
                                <div className="form-field-wrapper col-span-1">
                                    <label className="form-label dark:text-gray-400">{t("SKU Code")}</label>
                                    <input type="text" value={formData.skuCode || ''} onChange={e => setFormData({...formData, skuCode: e.target.value})} className="form-input bg-transparent" placeholder={t("e.g. SKU-123...")} />
                                </div>
                                <div className="form-field-wrapper col-span-1">
                                    <label className="form-label dark:text-gray-400">{t("Item Code")}</label>
                                    <select value={formData.itemCode || ''} onChange={e => setFormData({...formData, itemCode: e.target.value})} className="form-input bg-white dark:bg-gray-800">
                                ...
                                <div className="form-field-wrapper col-span-1">
                                    <label className="form-label dark:text-gray-400">{t("Barcode / EAN / UPC")}</label>
                                    <input type="text" value={formData.barcode || ''} onChange={e => setFormData({...formData, barcode: e.target.value})} className="form-input bg-transparent font-mono" placeholder={t("Scan or enter barcode...")} />
                                </div>
                                <div className="form-field-wrapper col-span-1">
                                    <label className="form-label dark:text-gray-400">{t("Alias / Alternate Name")}</label>
                                    <input type="text" value={formData.alias || ''} onChange={e => setFormData({...formData, alias: e.target.value})} className="form-input bg-transparent" placeholder={t("Other known name...")} />
                                </div>
            ...
            {deleteConfirmation?.isOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95 dark:bg-gray-800">
                        ...
                        <h2 className="font-bold text-xl mb-2 text-gray-900 dark:text-white">{t("Delete SKU?")}</h2>
