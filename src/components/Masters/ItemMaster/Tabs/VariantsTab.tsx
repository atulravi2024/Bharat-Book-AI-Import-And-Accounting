        ...
        <div className="flex flex-col h-full animate-in fade-in duration-300">
            <div className="p-4 bg-gray-50/30 border-b border-gray-100 flex justify-between items-center dark:bg-gray-800/30 dark:border-gray-800">
                <div className="relative max-w-md w-full mr-4">
                    ...
                    <input type="text" placeholder={t("Search Variants...")} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="form-input pl-10 pr-4 text-sm" />
                </div>
                <div className="flex items-center">
                    <ImportExportButtons data={data} onSave={onSave} entityName="VariantsTab" />
                    ...
                    <AddIcon className="lg:mr-2" /> <span className="hidden lg:inline-block">{t("Add Variant")}</span></button>
                </div>
            </div>
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200 dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Color")}</th>
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Size")}</th>
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Price Mod")}</th>
                    ...
                    <div className={`bg-white w-full h-full overflow-hidden flex flex-col dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-2xl animate-in zoom-in-95 ${formSettings.currentModalMode === 'fullscreen' ? 'rounded-none max-w-full max-h-full' : 'rounded-2xl max-w-5xl max-h-[90vh]'}`}>
                        <div className="flex justify-between items-center px-4 py-2 border-b border-gray-100 bg-gray-50/50 dark:border-gray-800">
                            <h2 className="font-bold text-base text-gray-900 flex items-center dark:text-white">
                                {editingId ? t('Edit') : t('Add')} {t('Variant')}
                            </h2>
...
<input type="text" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} className="form-input" placeholder={t("Enter name...")} />
                                ...
                                <div className="form-field-wrapper col-span-1">
                                    <label className="form-label dark:text-gray-400">{t("Variant SKU Code")}</label>
                                    <input type="text" value={formData.skuCode || ''} onChange={e => setFormData({...formData, skuCode: e.target.value})} className="form-input bg-transparent" placeholder={t("e.g. SHIRT-RED-L...")} />
                                </div>
                                <div className="form-field-wrapper col-span-1">
                                    <label className="form-label dark:text-gray-400">{t("Base Item ID (Optional Reference)")}</label>
                                    <select value={formData.baseItemId || ''} onChange={e => setFormData({...formData, baseItemId: e.target.value})} className="form-input bg-white dark:bg-gray-800">
                                        <option value="">{t("Select Base Item...")}</option>
                                        {itemMasters.map(i => (
                                ...
                                <div className="form-field-wrapper col-span-1">
                                    <label className="form-label dark:text-gray-400">{t("Color Reference (Name/ID)")}</label>
                                    <select value={formData.colorId || ''} onChange={e => setFormData({...formData, colorId: e.target.value})} className="form-input bg-white dark:bg-gray-800">
                                ...
                                <div className="form-field-wrapper col-span-1">
                                    <label className="form-label dark:text-gray-400">{t("Size Reference (Name/ID)")}</label>
                                    <select value={formData.sizeId || ''} onChange={e => setFormData({...formData, sizeId: e.target.value})} className="form-input bg-white dark:bg-gray-800">
                                ...
                                <div className="form-field-wrapper col-span-1 md:col-span-2 mt-2">
                                    <label className="block text-xs font-bold text-green-600 uppercase mb-1 dark:text-green-500">{t("Price Modifier (Difference from base price)")}</label>
                                    <div className="flex items-center">
                                        ...
                                        <input type="number" value={formData.priceModifier || ''} onChange={e => setFormData({...formData, priceModifier: parseFloat(e.target.value)})} className="form-input rounded-r-lg bg-transparent" placeholder={t("0.00 (e.g. 50 for +50)")} />
                                    </div>
                                    <p className="text-xs text-gray-400 mt-1">{t("Specify amount to add or subtract (e.g. -10 or 50) from the base item's price.")}</p>
                                </div>
            ...
            {deleteConfirmation?.isOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95 dark:bg-gray-800">
                        ...
                        <h2 className="font-bold text-xl mb-2 text-gray-900 dark:text-white">{t("Delete Variant?")}</h2>
