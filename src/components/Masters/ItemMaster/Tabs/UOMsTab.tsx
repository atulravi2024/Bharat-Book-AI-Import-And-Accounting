    ...
    return (
        <div className="flex flex-col h-full animate-in fade-in duration-300">
            <div className="p-4 bg-gray-50/30 border-b border-gray-100 flex justify-between items-center dark:bg-gray-800/30 dark:border-gray-800">
                <div className="relative max-w-md w-full mr-4">
                    ...
                    <input type="text" placeholder={t("Search UOMs...")} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="form-input pl-10 pr-4 text-sm" />
                </div>
                <div className="flex items-center">
                    ...
                    <AddIcon className="lg:mr-2" /> <span className="hidden lg:inline-block">{t("Add UOM")}</span></button>
                </div>
            </div>
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200 dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Symbol")}</th>
                                <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">{t("Conversion")}</th>
        ...
        <td className="p-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-200">
            {m.baseUom ? (
                ...
                <span className="text-xs text-gray-400 italic">{t("Base Unit")}</span>
            )}
        </td>
                                    <td className="p-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-200">
...
<label className="form-label dark:text-gray-400">{t("Symbol")}</label>
            <input type="text" value={formData.symbol || ''} onChange={e => setFormData({...formData, symbol: e.target.value})} className="form-input bg-transparent" placeholder={t("e.g. KG, PCS...")} />
        </div>
        
        <div className="form-field-wrapper col-span-1 md:col-span-2 border-t border-gray-100 dark:border-gray-700 pt-4 mt-2">
            <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 mb-3">{t("Unit Conversion")}</h3>
            <p className="text-xs text-gray-500 mb-4">{t("Set this if this unit is a multiple or fraction of another base unit (e.g., 1 Box = 10 Pcs).")}</p>
            <div className="form-grid gap-4">
                <div className="form-field-wrapper">
<label className="form-label dark:text-gray-400">{t("Base UOM")}</label>
                    <select value={formData.baseUom || ''} onChange={e => setFormData({...formData, baseUom: e.target.value})} className="form-input">
                        <option value="">{t("None (This is a Base Unit)")}</option>
                ...
                {formData.baseUom && (
                    <div className="form-field-wrapper">
<label className="form-label dark:text-gray-400">{t("Conversion Factor")}</label>
                        <div className="flex items-center">
                            <span className="text-sm font-medium mr-2 whitespace-nowrap">1 {formData.symbol || formData.name || 'Unit'} =</span>
                            <input type="number" value={formData.conversionFactor || ''} onChange={e => setFormData({...formData, conversionFactor: parseFloat(e.target.value)})} className="form-input w-24 bg-transparent text-center font-bold" placeholder={t("1.0")} />
            ...
            {deleteConfirmation?.isOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95 dark:bg-gray-800">
                        ...
                        <h2 className="font-bold text-xl mb-2 text-gray-900 dark:text-white">{t("Delete UOM?")}</h2>
