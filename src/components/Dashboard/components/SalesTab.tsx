    ...
    return (
        <motion.div key="sales" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <KPIComponent label={t("Total Sales")} val={`₹${formatNumber(Number(stats.vol.sales))}`} sub={t("Total revenue")} icon={TrendingUp} color="text-blue-600" bg="bg-blue-50" isDemo={isDemo} />
                <KPIComponent label={t("Sales Count")} val={formatNumber(stats.counts.sales)} sub={t("Total invoices")} icon={Activity} color="text-indigo-600" bg="bg-indigo-50" isDemo={isDemo} />
                <KPIComponent label={t("Average Sales")} val={`₹${formatNumber(stats.counts.sales > 0 ? stats.vol.sales / stats.counts.sales : 0, { maximumFractionDigits: 0 })}`} sub={t("Per invoice")} icon={Target} color="text-emerald-600" bg="bg-emerald-50" isDemo={isDemo} />
                <KPIComponent label={t("Estimated Tax")} val={`₹${formatNumber(stats.vol.sales * 0.18, { maximumFractionDigits: 0 })}`} sub={t("Assuming 18% GST")} icon={Zap} color="text-amber-600" bg="bg-amber-50" isDemo={isDemo} />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] shadow-sm border border-premium-slate-100 h-[400px] relative dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <h3 className="text-gray-500 font-medium mb-6 uppercase tracking-wider text-xs">{t("Sales Trend")}</h3>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-premium-slate-100 h-[400px] relative flex flex-col dark:bg-gray-800 dark:border-gray-700">
                    <h3 className="text-gray-500 font-medium mb-2 uppercase tracking-wider text-xs">{t("Top Customers")}</h3>
                    {stats.topPartiesSales?.length > 0 ? (
                        <div className="flex-1 min-h-0 relative">
                            <SafeResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                                <PieChart>
                                    <Pie
                                        data={stats.topPartiesSales}
                                        cx="50%"
                                        cy="50%"
                                        innerRadius={60}
                                        outerRadius={90}
                                        ...
                                        dataKey="value"
                                    >
                                        {stats.topPartiesSales.map((entry: any, index: number) => (
