...
export const TrialBalance: React.FC<TrialBalanceProps> = ({ data }) => {
      ...
      <table className="w-full text-sm text-left">
        <thead>
          <tr className="bg-gray-50 border-b dark:bg-gray-900">
            ...
            <th className="px-4 py-3 font-bold text-gray-600 uppercase text-[10px] dark:text-gray-300">{t("Debit/Credit Ledger")}</th>
        ...
        <tbody>
          {data.map(v => (
            <tr key={v.id} className="border-b hover:bg-gray-50 transition-colors dark:hover:bg-gray-700">
              ...
              <td className="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">{String(v.ledger?.value || v.debitLedger?.value || v.creditLedger?.value || '-')}</td>
          ...
          {data.length === 0 && (
            <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-400">{t("No records found for this period.")}</td></tr>
          )}
        </tbody>
        {data.length > 0 && (
          <tfoot>
            <tr className="bg-gray-50 font-bold border-t-2 border-gray-200 dark:bg-gray-900 dark:border-gray-700">
              <td colSpan={4} className="px-4 py-4 text-right uppercase text-[10px] tracking-widest text-gray-500 dark:text-gray-400">{t("Total")}</td>
