...
import {
  calculateRowAmountBeforePreTaxRoundOff,
  calculateRowAmount,
  getRowPostTaxDiscount,
  getRowRoundOff,
  getRowPreTaxRoundOff,
} from '../../VoucherCalculations';

interface UseCreditNoteVoucherTotalsProps {
  rows: any[];
  activeTab: string;
  headerDetails: any;
}

export const useCreditNoteVoucherTotals = ({
  rows,
  activeTab,
  headerDetails,
}: UseCreditNoteVoucherTotalsProps) => {
  return useMemo(() => {
    if (
      activeTab === 'sales' ||
      activeTab === 'purchase' ||
      ...
      const _place = String(
        headerDetails.placeOfSupply?.value || headerDetails.placeOfSupply || ''
      )
        .trim()
        .toLowerCase();
      ...
      if (_ledger.includes('igst') || _ledger.includes('inter')) {
        ...
        _ledger.includes('cgst') ||
        _ledger.includes('sgst') ||
        _ledger.includes('local') ||
        _ledger.includes('intra') ||
        (_ledger.includes('gst') && !_ledger.includes('igst'))
      ) {
      ...
      if (headerDetails.taxableOtherAdjustmentPct) {
        taxableOtherAdjustmentAmount +=
          amountAfterDiscount * (parseFloat(headerDetails.taxableOtherAdjustmentPct) / 100);
      }
      ...
      if (headerDetails.nonTaxableOtherAdjustmentPct) {
        nonTaxableOtherAdjustmentAmount +=
          amountAfterDiscount * (parseFloat(headerDetails.nonTaxableOtherAdjustmentPct) / 100);
      }
      ...
      if (headerDetails.nonTaxableVoucherDiscountPct) {
        nonTaxableVoucherDiscountAmt +=
          amountAfterDiscount * (parseFloat(headerDetails.nonTaxableVoucherDiscountPct) / 100);
      }
      if (headerDetails.nonTaxableVoucherDiscountAmount) {
        nonTaxableVoucherDiscountAmt +=
          parseFloat(headerDetails.nonTaxableVoucherDiscountAmount) || 0;
      }
      ...
      const getCrDr = (r: any, i: number) =>
        r.crDr ||
        (activeTab === 'payment' && i === 0
          ? 'Cr'
          : activeTab === 'payment'
            ? 'Dr'
            : activeTab === 'receipt' && i === 0
              ? 'Dr'
              : activeTab === 'receipt'
                ? 'Cr'
                : activeTab === 'journal'
                  ...
                  : 'Cr');
      const drTotal = rows
        .filter((r, i) => getCrDr(r, i) === 'Dr')
        .reduce((s, r) => s + (parseFloat(r.amount) || 0), 0);
      const crTotal = rows
        .filter((r, i) => getCrDr(r, i) === 'Cr')
        .reduce((s, r) => s + (parseFloat(r.amount) || 0), 0);

      const total =
        activeTab === 'payment' ? crTotal : activeTab === 'receipt' ? drTotal : Math.max(drTotal, crTotal);
      return {
        subtotal: total,
