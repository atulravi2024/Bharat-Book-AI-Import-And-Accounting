...
import { usePurchaseBarcodeScanner } from './usePurchaseBarcodeScanner';
import { usePurchaseVoucherTotals } from './usePurchaseVoucherTotals';
import { usePurchaseVoucherNavigation } from './usePurchaseVoucherNavigation';



export const usePurchaseVoucherLogic = (props: VoucherEntryViewProps) => {
  ...
  const activeTab = 'purchase' as string;
  ...
  const {
    ...
    rows,
    setRows,
    handleItemOrSkuChange: (rowIndex, value, field) => handleItemOrSkuChange(rowIndex, value, field),
  });
  ...
  useEffect(() => {
    // Auto-detect supply type based on Place of Supply, Ledger and GSTIN
    const currentPlace = String(headerDetails.placeOfSupply?.value || headerDetails.placeOfSupply || '').trim().toLowerCase();
    const ledger = (headerDetails.purchaseLedger || '').toLowerCase();
    const gstinValue = String(headerDetails.gstNumber?.value || headerDetails.gstNumber || '').trim();
    
    // Check keywords in ledger first as it's a direct accounting directive
    const hasIgstKeyword = ledger.includes('igst') || ledger.includes('inter');
    const hasCgstKeyword = ledger.includes('cgst') || ledger.includes('sgst') || ledger.includes('local') || ledger.includes('intra') || (ledger.includes('gst') && !ledger.includes('igst'));

    if (hasIgstKeyword) {
      if (headerDetails.supplyType !== 'Inter-State') handleHeaderChange('supplyType', 'Inter-State');
      return;
    }
    if (hasCgstKeyword) {
    ...
    if (headerDetails.supplyType !== 'Intra-State') handleHeaderChange('supplyType', 'Intra-State');
  }, [headerDetails.placeOfSupply, headerDetails.purchaseLedger, headerDetails.gstNumber]);
  ...
  const totals = usePurchaseVoucherTotals({
    rows,
    activeTab,
    headerDetails,
  });
