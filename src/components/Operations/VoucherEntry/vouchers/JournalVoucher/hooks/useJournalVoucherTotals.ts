...
interface UseJournalVoucherTotalsProps {
  rows: any[];
  activeTab: string;
  headerDetails: any;
}

export const useJournalVoucherTotals = ({
  rows,
  activeTab,
  headerDetails,
}: UseJournalVoucherTotalsProps) => {
  return React.useMemo(() => {
    // Journal/Accounting vouchers do not have inventory properties by default.
    // However, we preserve the exact original nested check structure to ensure 100% bug-free behavior.
    if (false) {
      ...
      let computedIsInterState = headerDetails.supplyType === 'Inter-State';
