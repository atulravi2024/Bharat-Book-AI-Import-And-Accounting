...
import { useJournalVoucherLogic } from './hooks/useJournalVoucherLogic';
import { AccountingVoucherProps } from './types';

export const JournalVoucher: React.FC<AccountingVoucherProps> = (props) => {
  const logic = useJournalVoucherLogic(props);
  ...
  const { type = 'journal', defaultType, initialVoucher, itemMasters = [], ledgerMasters = [], partyMasters = [], vouchers = [], onUpdateItemMaster, onAddItemMaster, onSaveEntry, onDeleteEntry, onOpenPrintSettings } = props;
...
<label className="form-label">{t("Account (Cash/Bank)")}</label>
                <input list="ledger-list" value={headerDetails.cashBankAccount || ''} onChange={(e) => handleHeaderChange('cashBankAccount', e.target.value)} placeholder={t("Search Cash/Bank Account...")} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all dark:bg-gray-900 dark:border-gray-700 dark:focus:bg-gray-700" />
              </div>
              <div className="form-field-wrapper">
...
return (
    <>
      ...
      <HistoryModal onDeleteRecord={onDeleteEntry} items={(vouchers || []).filter(v => ['journal', 'general'].includes(typeof v.type === 'string' ? v.type.toLowerCase().replace(/[\s_]+/g, '') : ''))} 
        isOpen={showHistory} 
