      ...
      <div className={`border-b border-gray-100 flex justify-between items-center bg-gray-50/50 cursor-pointer ${collapsedSections.lineItems ? 'px-4 py-3' : 'px-6 py-5'}`} onClick={() => toggleSection('lineItems')}>
        <h3 className="text-sm font-black text-gray-800 uppercase tracking-widest flex items-center">
         ...
         <div className="flex gap-2 items-center">
           ...
           <button onClick={(e) => { e.stopPropagation(); }} className="flex items-center px-3 md:px-3 py-2 md:py-1.5 bg-white text-gray-600 border border-gray-200 rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-gray-50 transition-colors" title="Import">
             <Import size={16} /> 
           </button>
           <button onClick={(e) => {
             e.stopPropagation();
             setShowNewItemModal(true);
           }} className="flex items-center px-4 md:px-3 py-2 md:py-1.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-lg text-xs font-black uppercase tracking-widest hover:bg-blue-100 transition-colors" title="New Item">
             <Settings2 size={16} className="md:mr-1" /> <span className="hidden md:inline">New Item</span>
           </button>
           <button onClick={(e) => { e.stopPropagation(); setRows([...rows, { id: Date.now() }]); }} className="flex items-center px-4 md:px-3 py-2 md:py-1.5 bg-purple-100 text-purple-700 rounded-lg text-xs font-black uppercase tracking-widest hover:bg-purple-200 transition-colors" title="Add Item">
        ...
        <table className="w-full text-sm text-left whitespace-nowrap">
          <thead className="bg-gray-50/80 border-b border-gray-100 uppercase text-[10px] tracking-[0.2em] font-black text-gray-400">
            <tr>
              ...
              <th className="px-6 py-4 min-w-[150px]">SKU</th>
              <th className="px-6 py-4 min-w-[120px] hidden md:table-cell">Batch/Lot</th>
              {activeTab === 'physical_stock' ? (
                <>
                  <th className="px-6 py-4 text-right min-w-[120px]">Book Qty</th>
                  <th className="px-6 py-4 text-right min-w-[120px]">Physical Qty</th>
                </>
              ) : (
                <th className="px-6 py-4 text-right min-w-[120px]">Quantity</th>
              )}
              <th className="px-6 py-4 min-w-[100px]">Unit</th>
              {activeTab !== 'physical_stock' && <th className="px-6 py-4 text-right min-w-[120px]">Rate/Value (₹)</th>}
