...
export const InventoryEntryView: React.FC<InventoryEntryViewProps> = ({ defaultType, itemMasters = [], warehouseMasters = [], ledgerMasters = [], partyMasters = [], onUpdateItemMaster, onAddItemMaster, onOpenPrintSettings }) => {
  ...
  const totals = React.useMemo(() => {
    ...
    if (headerDetails.otherAdjustmentPct) {
      otherAdjustmentAmount += estValue * (parseSafe(headerDetails.otherAdjustmentPct) / 100);
    }
    if (headerDetails.otherAdjustment) {
      otherAdjustmentAmount += parseSafe(headerDetails.otherAdjustment);
    }
    
    const finalVal = estValue - voucherDiscountAmt + otherAdjustmentAmount;
    const finalValue = isFinite(finalVal) && !isNaN(finalVal) ? Math.max(0, finalVal) : 0;
    ...
    <div className="max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 animate-in fade-in duration-500 min-h-screen pb-32">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4 border-b border-gray-200 pb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-2 font-medium">Manage item movements, adjustments, and non-monetary stock updates</p>
        </div>
