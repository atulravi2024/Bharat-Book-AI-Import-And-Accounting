...
export const VoucherEntryView: React.FC<VoucherEntryViewProps> = ({ defaultType, initialVoucher, itemMasters = [], ledgerMasters = [], partyMasters = [], onUpdateItemMaster, onAddItemMaster, onSaveEntry, onOpenPrintSettings }) => {
  const [activeTab, setActiveTab] = useState(defaultType || 'sales');
  
  useEffect(() => {
    if (!initialVoucher && defaultType) {
  ...
  React.useEffect(() => {
    if (initialVoucher) {
      ...
      if (initialVoucher.header && initialVoucher.rows) {
        setHeaderDetails(initialVoucher.header);
  ...
  const getRowPostTaxDiscount = (row: any) => {
    ...
    const taxPct = parseSafe(row.tax);
  ...
  const totals = React.useMemo(() => {
    if (activeTab === 'sales' || activeTab === 'purchase' || activeTab === 'debit_note' || activeTab === 'credit_note') {
      ...
      const isInterState = headerDetails.placeOfSupply === 'other_state';
      ...
      let otherAdjustmentAmount = 0;
      if (headerDetails.otherAdjustmentPct) {
        otherAdjustmentAmount += amountAfterDiscount * (parseFloat(headerDetails.otherAdjustmentPct) / 100);
      }
      if (headerDetails.otherAdjustment) {
        otherAdjustmentAmount += parseFloat(headerDetails.otherAdjustment) || 0;
      }
      ...
      const grandTotalExact = amountAfterDiscount + taxAmount - postTaxDiscountSum + rowRoundOffs + otherAdjustmentAmount - voucherDiscountAmt;
      const grandTotalRound = Math.round(grandTotalExact);
      const globalRoundOff = grandTotalRound - grandTotalExact;
      ...
      return { 
        ...
        voucherDiscount: voucherDiscountAmt,
        cgst, 
        sgst, 
        igst, 
        roundOff: globalRoundOff + rowRoundOffs + preTaxRoundOffs, 
        otherAdjustment: otherAdjustmentAmount,
  ...
  const loadRecord = (voucher: any | null) => {
    ...
    if (voucher.header && voucher.rows) {
      setHeaderDetails(voucher.header);
  ...
  const renderSalesPurchaseForm = () => (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="lg:col-span-2">
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
              ...
              <div className="sm:w-40 flex items-center justify-center px-4 py-3 bg-blue-50 border border-blue-100/50 rounded-xl text-sm font-black text-blue-700 shadow-sm shrink-0 whitespace-nowrap uppercase tracking-widest ring-1 ring-blue-200/50">
                  {(() => {
          ...
          <div className="lg:col-span-3 hover:bg-gray-50 flex items-center p-2 rounded-xl border border-transparent transition-all">
      ...
      <div className={`bg-white border border-gray-200/60 shadow-sm relative transition-all duration-300 z-[40] ${collapsedSections.party ? 'px-6 py-3 rounded-xl' : 'p-6 rounded-2xl'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div>
          ...
          <div className="bg-blue-50/30 p-6 rounded-2xl border border-blue-100/50">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              {headerDetails.isEWayBillRequired && (
                <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4 animate-in zoom-in-95 duration-300">
                  <div>
        ...
        <div className={`border-b border-gray-100 flex justify-between items-center bg-gray-50/50 cursor-pointer ${collapsedSections.lineItems ? 'px-4 py-3' : 'px-6 py-5'}`} onClick={() => toggleSection('lineItems')}>
          <h3 className="text-sm font-black text-gray-800 uppercase tracking-widest flex items-center">
           ...
           <div className="flex items-center space-x-2 md:space-x-3">
             ...
             <button onClick={(e) => { e.stopPropagation(); }} className="flex items-center px-4 md:px-3 py-2 md:py-1.5 bg-white text-gray-600 border border-gray-200 rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-gray-50 transition-colors" title="Import">
               <Import size={16} /> 
             </button>
             <button onClick={(e) => {
               e.stopPropagation();
  ...
  const renderAccountingForm = () => (
    <div className="space-y-6">
      ...
      <div className={`bg-white border border-gray-200/60 shadow-sm relative transition-all duration-300 z-[50] ${collapsedSections.header ? 'px-6 py-3 rounded-xl' : 'p-6 rounded-2xl'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-top-2 duration-300">
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
              <input type="date" value={headerDetails.voucherDate} onChange={(e) => handleHeaderChange('voucherDate', e.target.value)} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all" />
      ...
      <div className={`bg-white border border-gray-200/60 shadow-sm flex flex-col relative transition-all duration-300 z-[40] ${collapsedSections.lineItems ? 'rounded-xl' : 'rounded-2xl'}`}>
                ...
                <tr key={row.id} className="hover:bg-amber-50/30 transition-colors group">
                  <td className="px-6 py-3">
                    <select 
                      ...
                      className="w-full px-2 py-2 bg-transparent border border-transparent group-hover:border-gray-200 rounded-lg text-sm font-black uppercase focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all cursor-pointer text-center disabled:opacity-50"
                    >
                  ...
                  <td className="px-6 py-3">
                    <SearchableDropdown
                      ...
                      buttonClassName="w-full min-w-[300px] px-3 py-2 bg-transparent border border-transparent group-hover:border-gray-200 rounded-lg text-sm font-medium focus-within:bg-white focus-within:outline-none focus-within:ring-2 focus-within:ring-amber-500/20 focus-within:border-amber-500 transition-all"
                    />
                  </td>
                  <td className="px-6 py-3">
                    <input type="number" placeholder="0.00" value={row.amount || ''} onChange={(e) => { const r = [...rows]; r[index].amount = e.target.value; setRows(r); }} className="w-full px-3 py-2 bg-transparent border border-transparent group-hover:border-gray-200 rounded-lg text-sm font-bold text-right focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all" />
                  </td>
  ...
  return (
    <div className="max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 animate-in fade-in duration-500 min-h-screen pb-32">
      ...
      <div className="mb-8 overflow-x-auto no-scrollbar w-full">
      ...
      <div className="flex flex-col gap-6 items-stretch">
        <div className="w-full">
          {(activeTab === 'sales' || activeTab === 'purchase' || activeTab === 'debit_note' || activeTab === 'credit_note') ? renderSalesPurchaseForm() : renderAccountingForm()}
