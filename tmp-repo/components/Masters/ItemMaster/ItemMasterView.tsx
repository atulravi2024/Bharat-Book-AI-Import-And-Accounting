            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="p-4 bg-gray-50/30 border-b border-gray-100">
                    <div className="relative max-w-md">
                        ...
                        <input type="text" placeholder={`Search ${activeTab}...`} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm" />
                    </div>
                </div>
                                    ...
                                    <tr key={m.id} className="hover:bg-blue-50/50 transition-colors group">
                                        <td className="p-4">
                                            <div className="flex items-center">
                                                ...
                                                <div>
                                                    <div className="font-bold text-gray-900 text-sm font-sans">{m.name || m.code}</div>
                                                    {activeTab === 'items' && m.sku && <div className="text-[11px] text-gray-500 font-mono mt-0.5">SKU: {m.sku}</div>}
                                                    {activeTab === 'items' && m.hsnCode && <div className="text-[11px] text-gray-500 font-mono">HSN: {m.hsnCode}</div>}
                                                </div>
                                            </div>
                                        </td>
                                        {activeTab === 'items' && (
                                            <>
                                                <td className="p-4 text-sm text-gray-700">
                                                    {m.category && <span className="inline-block px-2 py-1 bg-gray-100 rounded text-[10px] font-bold uppercase text-gray-600 mb-1">{m.category}</span>}
                                                    {m.brand && <div className="text-xs text-gray-500">{m.brand}</div>}
                                                </td>
                                                <td className="p-4 text-sm text-gray-700">
                                                    {m.uom && <div>UOM: <span className="font-mono">{m.uom}</span></div>}
                                                    {m.stockGroup && <div className="text-xs text-gray-500">{m.stockGroup}</div>}
                                                </td>
                                                <td className="p-4 text-sm text-gray-700">
                                                ...
                                                <td className="p-4 text-sm text-gray-700">
                                                    <div className="font-mono font-medium text-gray-900">{m.salesRate ? `₹${m.salesRate.toFixed(2)}` : '-'}</div>
                                                    {m.purchaseRate ? <div className="text-[11px] text-gray-500 font-mono mt-0.5">Cost: ₹{m.purchaseRate.toFixed(2)}</div> : null}
                                                </td>
                                            </>
                                        )}
                                        {activeTab === 'uoms' && <td className="p-4 text-sm text-gray-700"><span className="font-mono bg-gray-100 px-2 py-1 rounded text-xs">{m.symbol || '-'}</span></td>}
                                        {activeTab === 'gst' && <td className="p-4 text-sm text-gray-700"><span className="font-mono bg-amber-50 text-amber-700 ring-1 ring-amber-100 px-2 py-1 rounded text-xs font-bold">{m.rate ? `${m.rate}%` : '-'}</span></td>}
                                        {activeTab === 'gst' && <td className="p-4 text-xs text-gray-700 uppercase tracking-widest">{m.type || '-'}</td>}
                                        {activeTab !== 'items' && <td className="p-4 text-xs text-gray-500">{m.description || '-'}</td>}
                                        <td className="p-4">
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-[1.25rem] w-full max-w-2xl shadow-2xl animate-in zoom-in-95 overflow-hidden flex flex-col max-h-[90vh]">
                        ...
                        <div className="overflow-y-auto flex-1 p-6 space-y-4 custom-scrollbar">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="col-span-1 md:col-span-2">
                                    ...
                                    <input type="text" value={formData.name || formData.code || ''} onChange={e => setFormData({...formData, name: e.target.value, code: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter name or code..." autoFocus />
                                </div>
                                <div className="col-span-1 md:col-span-2">
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Description / Notes</label>
                                    <input type="text" value={formData.description || ''} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Add any extra details..." />
                                </div>
                                ...
                                {activeTab === 'items' && (
                                    <>
                                        <div>
                                            ...
                                            <input type="text" value={formData.sku || ''} onChange={e => setFormData({...formData, sku: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter SKU..." />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Unit of Measure (UOM)</label>
                                            <select value={formData.uom || ''} onChange={e => setFormData({...formData, uom: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" value={formData.taxRate || 0} onChange={e => setFormData({...formData, taxRate: parseFloat(e.target.value) || 0})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">HSN Code</label>
                                            <input type="text" value={formData.hsnCode || ''} onChange={e => setFormData({...formData, hsnCode: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter HSN..." />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Category</label>
                                            <select value={formData.category || ''} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                                        ...
                                        <div>
                                            ...
                                            <select value={formData.brand || ''} onChange={e => setFormData({...formData, brand: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                                                <option value="">Select Brand...</option>
                                                {brandMasters.map(b => <option key={b.id} value={b.name}>{b.name}</option>)}
                                            </select>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Purchase Rate</label>
                                            <input type="number" value={formData.purchaseRate || ''} onChange={e => setFormData({...formData, purchaseRate: parseFloat(e.target.value)})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="0.00" />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Sales Rate</label>
                                            <input type="number" value={formData.salesRate || ''} onChange={e => setFormData({...formData, salesRate: parseFloat(e.target.value)})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="0.00" />
                                        </div>
                                    </>
                                )}
                                ...
                                {activeTab === 'uoms' && (
                                    <div>
                                        ...
                                        <input type="text" value={formData.symbol || ''} onChange={e => setFormData({...formData, symbol: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. KG, PCS..." />
                                    </div>
                                )}
                                ...
                                {activeTab === 'gst' && (
                                    <>
                                        <div>
                                            ...
                                            <input type="number" value={formData.rate || 0} onChange={e => setFormData({...formData, rate: parseFloat(e.target.value) || 0})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Type</label>
                                            <select value={formData.type || 'Goods'} onChange={e => setFormData({...formData, type: e.target.value as 'Goods'|'Services'})} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                        ...
                        <div className="flex space-x-3 p-6 border-t border-gray-100 bg-gray-50/50">
                             <button onClick={() => setIsModalOpen(false)} className="flex-1 py-3 bg-white border border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition-all active:scale-95 text-sm">Cancel</button>
            ...
            {deleteConfirmation?.isOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95">
                        ...
                        <h2 className="font-bold text-xl mb-2 font-display text-gray-900">Delete {activeTab.slice(0, activeTab.endsWith('s') ? -1 : undefined)}?</h2>
                        <p className="text-gray-500 mb-6 text-sm">Are you sure you want to delete <span className="font-bold text-gray-900">"{deleteConfirmation.name}"</span>? This action cannot be undone.</p>
                        <div className="flex space-x-3">
                             <button onClick={() => setDeleteConfirmation(null)} className="flex-1 py-3 bg-white border border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition-all active:scale-95 text-sm">Cancel</button>
